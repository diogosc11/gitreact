...
import {Title, Text, Button, Input} from '../Drawer/styled';
  ...
  return (
    <div className={classes.root}>
      ...
      >
        ...
        <Divider />
        <List>
          {['Upload de nudes', 'Nudes via email'].map((text, index) => (
        ...
        <Divider />
        <List>
          {['Pegar ônibus', 'Procurar namorada'].map((text, index) => (
            <ListItem button key={text}>
              <ListItemIcon>{index % 2 === 0 ? <AirportShuttleIcon /> : <FavoriteIcon />}</ListItemIcon>
              <ListItemText primary={text} />
            </ListItem>
          ))}
        </List>
      ...
      <main className={classes.content}>
        ...
        <Input></Input>
        <Button><strong>Adicionar tarefa</strong></Button>
        <Footer />
      </main>
    </div>
  );
